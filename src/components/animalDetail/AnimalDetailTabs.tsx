...
const AnimalDetailTabs: React.FC<AnimalDetailTabsProps> = ({ animalId, animalData }) => {
    ...
    // Hayvan sensör pil seviyesi (örnek veri ya da animalData'dan al)
    const batteryLevel = animalData.batteryLevel || 78;

    // animalDetailMocks ile mevcut animalData nesnelerini birleştir
    const mockData = animalDetailMocks || {};

    // animal.name ve tagNumber verilerini burada göster
    const animalName = animalData.name || "İsimsiz Hayvan";
    const animalTagNumber = animalData.tagNumber || animalId;
    const animalSpecies = animalData.species || "Bilinmiyor";
    const animalBreed = animalData.breed || "";
    const animalGender = animalData.gender || "Bilinmiyor";
    const animalAge = animalData.age || "?";
    const animalWeight = animalData.weight || "?";
            ...
            <div className="animal-header">
                <div className="animal-header-left">
                    <div className="header-title-row">
                        <h1 className="animal-name">{animalName} ({animalTagNumber})</h1>
